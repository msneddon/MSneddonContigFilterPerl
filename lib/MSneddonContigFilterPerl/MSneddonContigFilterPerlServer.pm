package MSneddonContigFilterPerl::MSneddonContigFilterPerlServer;
...
sub get_service_name
{
    ...
    if(!defined $ENV{$SERVICE}) {
        return 'MSneddonContigFilterPerl';
    }
    return $ENV{$SERVICE};
}
...
sub call_method {
    ...
    my $ctx = MSneddonContigFilterPerl::MSneddonContigFilterPerlServerContext->new($self->{loggers}->{userlog},
...
{
    # Service MSneddonContigFilterPerl requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for MSneddonContigFilterPerl but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = MSneddonContigFilterPerl::MSneddonContigFilterPerlServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service MSneddonContigFilterPerl.");
    }
...
package MSneddonContigFilterPerl::MSneddonContigFilterPerlServerContext;

use strict;

=head1 NAME

MSneddonContigFilterPerl::MSneddonContigFilterPerlServerContext
...
package MSneddonContigFilterPerl::MSneddonContigFilterPerlServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = MSneddonContigFilterPerl::MSneddonContigFilterPerlServer->new(
